...
* A time limit of one minute is sufficient for tests without `long` tag on JUWELS Booster and LUMI.
...
[NOTE]
====
Given the same source code and configuration, clang-format does _not_ promise to create the same formatting across different versions. The following clang-format major releases were tested in libnegf#117 and generate the same output unless noted otherwise:

* 11 (Debian 11, *not* working)
* 14 (Debian 12)
* 18 (JUWELS Booster, Stages/2025)
* 19 (Debian 13)
* 20 (JUWELS Booster, Stages/2026)
====
