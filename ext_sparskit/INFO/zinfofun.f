...
c-----------------------------------------------------------------------
      nnz    = ia(n+1)-ia(1)
      call zcsrcsc(n,1,1,a,ja,ia,ao,jao,iao)
      if (sym) goto 7
      st     = 0.0d0
      fas    = 0.0d0
      fan    = 0.0d0
...
c     
         ...
         j2 = jao(k2)
         if (j1 .ne. j2 ) goto 51
         fas = fas + abs(a(k1)+ao(k2))**2
         fan = fan + abs(a(k1)-ao(k2))**2
         st  = st + abs(a(k1))**2
         imatch = imatch + 1
 51      k1 = k1+1
         k2 = k2+1
 ...
 6    continue
      fas = 0.25D0 * fas
      fan = 0.25D0 * fan
 7    call zfrobnorm(n,sym,ao,jao,iao,Fnorm)
      if (sym) then
         imatch = nnz
         fas = Fnorm
         fan = 0.0d0
...
c nskyu = skyline storage for upper part
      integer i,j0,j0r,lminc,jminr,jminc
...
c-----------------------------------------------------------------------
      integer n2,itot,ii,idiag,jmax,k,j,i 
