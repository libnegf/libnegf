...
!
  ...
  TYPE(z_DNS), DIMENSION(:,:,:), ALLOCATABLE, SAVE :: DDn
  TYPE(z_DNS), DIMENSION(:,:,:), ALLOCATABLE, SAVE :: DDp
  TYPE(z_DNS), DIMENSION(:,:,:), ALLOCATABLE, SAVE :: DDr
  TYPE(z_DNS), DIMENSION(:,:,:), ALLOCATABLE, SAVE :: Pin
  TYPE(z_DNS), DIMENSION(:,:,:), ALLOCATABLE, SAVE :: Pip
  TYPE(z_DNS), DIMENSION(:,:,:), ALLOCATABLE, SAVE :: Pir
...
CONTAINS
  ...
  !****************************************************************************
  ! 
  ! Driver for computing Equilibrium Green Retarded 
  !
  !****************************************************************************
  ...
  SUBROUTINE calls_Dr_ph(pnegf,E,SelfEneR,struct,comp_col,A)
    ...
    TYPE(z_CSR) :: ESH_tot 
    INTEGER :: i, m, cb, ierr, nbl, rbl, lbl, ncont
    ...
    IF (pnegf%phph%include_phph) THEN
      ...
      ENDDO
      DO i = 1, nbl-1
         call write_blkmat(Gr(i,i+1),pnegf%scratch_path,'G_r_',i,i+1,pnegf%iE)
         call write_blkmat(Gr(i+1,i),pnegf%scratch_path,'G_r_',i+1,i,pnegf%iE)
      ENDDO
    END IF
    ...
    IF (comp_col) THEN
      DO m=1,ncont
        cb = cblk(m)
        DO i = 1, cb-2
          call write_blkmat(Gr(i,cb),pnegf%scratch_path,'G_r_',i,cb,pnegf%iE)
        ENDDO
        DO i = cb+2, nbl
          call write_blkmat(Gr(i,cb),pnegf%scratch_path,'G_r_',i,cb,pnegf%iE)
        ENDDO
  ...
  !****************************************************************************
  !
  ! Driver for computing G_n including all contacts and phph scattering:
  !
  !****************************************************************************

  SUBROUTINE calls_Dn_ph(pnegf,E,SelfEneR,struct,Gout)
...
!          enddo
 ...
 SUBROUTINE create_scratch(nbl, npoints)
   ...
   ALLOCATE(DDn(nbl,nbl,npoints),stat=err)
   ALLOCATE(DDr(nbl,nbl,npoints),stat=err)
   ALLOCATE(DDp(nbl,nbl,npoints),stat=err)
   ALLOCATE(Pin(nbl,nbl,npoints),stat=err)
   ALLOCATE(Pir(nbl,nbl,npoints),stat=err)
   ALLOCATE(Pip(nbl,nbl,npoints),stat=err)
 ...
 SUBROUTINE destroy_scratch(nbl, npoints)
     ...
     do j = 1, nbl
       do iE = 1, npoints
           ...
           if (allocated(DDn)) then
               if (allocated(DDn(i,j,iE)%val)) call destroy(GGn(i,j,iE))
           endif       
           if (allocated(DDr)) then
               if (allocated(DDr(i,j,iE)%val)) call destroy(GGr(i,j,iE))
           endif       
           if (allocated(DDp)) then
               if (allocated(DDp(i,j,iE)%val)) call destroy(GGp(i,j,iE))
           endif       
           if (allocated(Pin)) then
               if (allocated(Pin(i,j,iE)%val)) call destroy(Sgn(i,j,iE))     
           endif       
           if (allocated(Pir)) then
               if (allocated(Pir(i,j,iE)%val)) call destroy(Sgr(i,j,iE))
           endif       
           if (allocated(Pip)) then
               if (allocated(Pip(i,j,iE)%val)) call destroy(Sgp(i,j,iE))     
           endif       
   ...
   if (allocated(DDn)) DEALLOCATE(GGn,stat=err)
   if (allocated(DDr)) DEALLOCATE(GGr,stat=err)
   if (allocated(DDp)) DEALLOCATE(GGp,stat=err)
   if (allocated(Pin)) DEALLOCATE(Sgn,stat=err)
   if (allocated(Pir)) DEALLOCATE(Sgr,stat=err)
   if (allocated(Pip)) DEALLOCATE(Sgp,stat=err)
 ...
 SUBROUTINE read_blkmat(Matrix, path, name, i, j, iE)
    ...
    if (memory) then
       ...
       case('G_n_')     
          Matrix%val = DDn(i,j,iE)%val 
       case('G_r_')     
          Matrix%val = DDr(i,j,iE)%val
       case('G_p_')     
          Matrix%val = DDp(i,j,iE)%val
       case('Sigma_ph_r_')     
          Matrix%val = Pir(i,j,iE)%val 
       case('Sigma_ph_n_')     
          Matrix%val = Pin(i,j,iE)%val
       case('Sigma_ph_p_')     
          Matrix%val = Pip(i,j,iE)%val
       case default 
  ...
  SUBROUTINE write_blkmat(Matrix, path, name, i, j, iE)
    ...
    if (memory) then
       ...
       select case(trim(name))
       case('G_n_')     
          if (.not.allocated(DDn(i,j,iE)%val)) then
              call create(DDn(i,j,iE),Matrix%nrow,Matrix%ncol)
          endif    
          DDn(i,j,iE)%val = Matrix%val
       case('G_r_')     
          if (.not.allocated(DDr(i,j,iE)%val)) then
              call create(DDr(i,j,iE),Matrix%nrow,Matrix%ncol)
          endif    
          DDr(i,j,iE)%val = Matrix%val
       case('G_p_')     
          if (.not.allocated(DDp(i,j,iE)%val)) then
              call create(DDp(i,j,iE),Matrix%nrow,Matrix%ncol)
          endif    
          DDp(i,j,iE)%val = Matrix%val 
       case('Sigma_ph_r_')     
          if (.not.allocated(Pir(i,j,iE)%val)) then
              call create(Pir(i,j,iE),Matrix%nrow,Matrix%ncol)
          endif    
          Pir(i,j,iE)%val = Matrix%val
       case('Sigma_ph_n_')     
          if (.not.allocated(Pin(i,j,iE)%val)) then
              call create(Pin(i,j,iE),Matrix%nrow,Matrix%ncol)
          endif    
          Pin(i,j,iE)%val = Matrix%val
       case('Sigma_ph_p_')     
          if (.not.allocated(Pip(i,j,iE)%val)) then
              call create(Pip(i,j,iE),Matrix%nrow,Matrix%ncol)
          endif    
          Pip(i,j,iE)%val = Matrix%val
       case default 
